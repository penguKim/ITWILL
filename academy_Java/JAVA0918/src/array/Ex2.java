...
//		arr2 = {1, 2, 3, 4, 5} // 오류 발생! 별도로 초기화 시 사용 불가능!
		...
		a= b; // b의 주소값을 a에 저장 
		// 기존에 a가 가리키전(참조하던) 1, 2, 3 저장 공간을 버리고
		// b가 가리키는 4, 5, 6 저장 공간의 주소값을 전달받아 참조하게 됨
		// 이 때, 기존 1, 2, 3 저장 공간은 더 이상 참조되지 않으므로
		// 불필요한 메모리 공간이 되어 Garbage Collector 에 의해 정리 대상이 됨
		// => 즉, 더 이상 필요없는 메모리 공간은 자동으로 정리됨
		
		b = c; // c의 주소값을 b에 저장
		// 기존에 b가 참조하던 4, 5, 6 저장 공간을 버리고
		// c가 기리키전 7, 8, 9 저장 공간의 주소값을 전달받아 참조
		// 이 떼. 기존 4, 5, 6 저장 공간은 여전이 a 에
	}
}
