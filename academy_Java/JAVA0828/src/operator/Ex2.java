...
public class Ex2 {
	...
	public static void main(String[] args) {
		 ...
		 * 			ex) char + int = (int)char + int = int + int = int
		 *
		 * - 규칙2) int 타입보다 큰 타입과의 연산 시 큰 타입으로 변환 후 수행
		 * 			=> 즉, 결과값이 무조건 큰 타입이 됨
		 * 			ex) int + long = (long)int + long = long + long = long
		 * 			ex) long + float = (float)long + float = float + float = float
		 * 
		 */
...
//		char ch2 = ch + 2; // 오류발생! char + int -> (int)char + int -> int
		...
		//    67에 해당하는 문자 'C'( 67을 char 타입으로 변환한 데이터) 가 저장돰
...
//		int i2 = i + l; // int + long = long 이므로 int 타입 변수에 저장 불가
		...
		// double 소수점 16~18자리
		// float 소수점 6~8자리
		// => float -> double 로 변환하면 double + double = double 이 된다
		...
		// 단, 리터럴끼리의 연상이라도 표현 가능 범위를 초과하면 오류 발생
