...
public class Ex2 {
	...
	public static void main(String[] args) {
		 ...
		 *      중복되는 문자열이 없으면 생성하고, 중복될 경우 주소값을 리턴므로
		 *      메모리 공간 낭비가 줄어들게 됨
		 ...
		 *      (첫번째 인덱스 0번, 마지막 글자 인덱스는 문자열 길이 -1)
		 *      ex) "Hello" 라는 문자열이 있다면 H = 0 ~ o = 4 의 인덱스를 가진다.
		 *      
		 * < String 클래스의 메서드 >
		...
		// => 문자열은 char[] 배열로 관리되며 5번 인덱스에 해당하는 'P'가 출력됨
		...
		// concat() : 문자열 결합(연결 연산자보다 연산 속도가 빠르다)
		...
		System.out.println("s1.indexOf('a') : " + s1.indexOf('x'));
		// => 존재하지 않는 문자이므로 -1 리턴
		System.out.println("s1.indexOf(\"a\") : " + s1.indexOf("a"));
		...
		//    Programming 부분의 'a' 가 탐색되어 인덱스 번호 10 리턴
		...
		System.out.println("---------------------------------------------");
		// 만약, "." 기호를 구분자로 지정하는 경우
		// 정규표현식에서 특수기호로 사용되어 문자열의 모든 문자를 구분자로 지정
...
//		String[] strArr = s4.split("."); // 모든 문자열이 제거되므로 사용 불가
		...
		// String.format() 메서드(= static 메서드)
		// 특정 문자열을 형식 지정문자(%?) 와 결합하여 형식을 갖춘 문자열로 리턴
		// => prinft() 메서드와 형식 지정문자 동일
		String name = "홍길동";
		int age = 20;
		double height = 180.7;
		...
		String s5 = "admin123!";
		char[] chArr = s5.toCharArray(); // a, d, m, i, m, 1, 2, 3, ! 로 분리되어 저장
