...
//		p.childPrn(); // 서브클래스에서 직접 정의한 메서드는 호출 불가!
		...
		// 슈퍼클래스타입 Parent 의 레퍼런스 변수(p) 생성
		Parent p;
		
		// 슈퍼클래스타입 변수 p에 서브클래스 타입 Child 인스턴스 주소를 전달(대입)
		p = c; // 묵시적 형변환(업캐스팅)
		// => Parent p = (Parent)c;
...
//		p.childPrn(); // 서브클래스에서 직접 정의한 메서드(접근 불가) => 오류 발생!
		...
		//    참조 영역에 대한 축소로 서브클래스 메소드가 보이지 않게 됨
...
//		p3.childPrn(); // 상속되지 않은(서브클래스에서 직접 정의한) 메서드 호출 불가
		...
		// 업캐스팅 된 Parent 타입 p3 의 인스턴스(주소값)를 Child 타입 변수 c3에 전달
//		Child c3 = p3; // 컴파일 에러 발생! 명시적(강제) 형변환 필수!
		...
		// Child 타입의 객체를 만든후 Parent 타입 참조변수에 저장 시(업캐스팅)
		// 메모리 영역에는 Child 클래스의 멤버가 포함된 객체가 만들어져
		// 이후 Child 타입 참조변수로 변환시 에러가 발생하지 않는다.
