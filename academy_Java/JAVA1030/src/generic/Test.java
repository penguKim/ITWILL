...
//			Person p = objArr[i]; // 다운캐스팅 필요
		...
		// 문자열 추가
		list.add("김희수"); // Object 타입이므로 Person 이 아닌 타입도 추가 가능
		// => Person 객체 형태로 꺼내서 사용하는 시점에서 문제가 발생할 수 있다.
		
		// 일반 for문을 통한 객체 출력
		for(int i = 0; i < list.size(); i++) { // size() 메서는는 length 처럼 길이를 리턴
//			Person p = list.get(i); // 인덱스에 해당하는 객체 리턴
			// => 제네릭 타입을 사용하지 않아 list 는 Object 로 되어 있음
			// => Object 타입을 Person 타입으로 대입하려고 하면 컴파일 에러 발생하며
			//    다운캐스팅이 필요함을 알 수 있음
			
//			Person p = (Person)list.get(i);
//			System.out.println(p);
			// => 실행을 시키면 다운캐스팅 코드에서 에러가 발생!
			// => java.lang.ClassCastException: class java.lang.String 
			//    cannot be cast to class generic.Person
			// => "김희수" 의 String 형태는 Person 객체로 변경할 수 없음을 알 수 있다.
