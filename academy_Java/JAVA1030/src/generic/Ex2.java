...
//	private static T stacicMember; // 컴파일 에러 발생!
	// => Cannot make a static reference to the non-static type T
	// => static 멤버변수에 제네릭타입 파라미터 사용 불가!
	// => 인스턴스 생성 시점보다 먼저 메모리에 로딩되므로 타입 변경 불가능

//	T instance = new T(); // 인스턴스 생성(new) 시 제네릭타입 파라미터로 생성자 호출 불가능
...
//		if(o instanceof T) { //instanceof 연산자에 제네릭타입 파라미터 사용 불가
			...
			// Use its erasure Object instead since further generic type information
			// will be erased at runtime
			// => 컴파일 시점에서 T 의 데이터타입 확인이 불가능하므로
			//    true, false 를 미리 판별할 수 없으며, 형변환 등의 수행이 불가능하다.
//		}
	}
