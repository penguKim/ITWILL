...
class Child extends Parent {
	...
	@Override
	public void watchTv() {
		System.out.println("서브클래스에서 오버라이딩 된 vatchTv() 메서드");
		System.out.println("독립한 나의 집에서 " + tv + " 보기!");
	}
	
	public void watchParentTv() {
		// 현재 클래스 내에서 오버라이딩으로 인해 은닉된 슈퍼클래스의 멤버는
		// 레퍼런스 super를 통해서 접근하면 사용이 가능하다!
		// => 레퍼런스 this 와 사용 방법이 동일
...
class ChildClass extends ParentClass {
	...
	public void method() {
		...
		// => 만약, ChildClass 클래스 멤버변수를 선언하지 않을 경우 super.x 와 동일	
		
		System.out.println("super.x = " + super.x); // 슈퍼클래스 멤버변수 x
		// => 만약, ParentClass 클래스 멤버변수를 선언하지 않을 경우 오류 발생!
		// => Parent 클래스의 슈퍼클래스인 Object 클래스에 존재하지 않는 멤버이기 때문
	}
}
